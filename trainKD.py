...
from distillation import DistillerIKD, DistillerRKD
...
def train(args, teachermodel):
    ...
    #teachermodel = nn.DataParallel(RAFT(args), device_ids=args.gpus)
    #print('TESTING OUTPUT========')
    
    #teachermodel.load_state_dict(torch.load(targs.model), strict=False)
    ...
    #print(f"TESTING OUTPUT  TRAIN LOADER ============ {type(test)} ")
    ...
    while should_keep_training:
        ...
        for i_batch, data_blob in enumerate(train_loader):
            ...
            #print(f'PREDICTION BASELINE ======{flow_predictions[0]}') 
            flow_predictions_teacher = teachermodel(image1, image2, iters=args.iters)  
            #print(f'PREDICTION TEACHER ====== {flow_predictions_teacher[0]}')   
            #print(f'PREDICTION COMPRESSED BASELINE ====== {flow_predictions_teacher.shape}') 

            if distilltype == 'rskd':
                for j in range(len(flow_predictions_teacher)):  
                    d = DistillerIKD(flow_predictions[j], flow_predictions_teacher[j] , temperature, l) 
                    distillation_loss += d.rskd_loss('s','t')
                loss, metrics = sequence_loss(flow_predictions, flow, valid, args.gamma)
                studentloss = loss
                loss = (1-alpha)*loss + alpha*(distillation_loss)
                print(f'test: {counter + 1}, Response based distiallation Loss: {distillation_loss} ======= Student Loss: {studentloss}====== General Loss {loss}')
                counter +=1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--norm', type=float, default=1, help='Define norm dimenssion to consider for the loss') # 1 or 2
